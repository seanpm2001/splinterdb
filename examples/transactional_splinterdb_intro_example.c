...
main()
{
   ...
   char fruit[USER_MAX_KEY_SIZE] = {0};
   ...
   slice       key   = slice_create(USER_MAX_KEY_SIZE, fruit);
   ...
   rc    = transactional_splinterdb_insert(spl_handle, &txn, key, value);
   printf("Inserted key '%s' value %lu\n", fruit, slice_length(value));
