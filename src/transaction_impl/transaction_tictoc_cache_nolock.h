...
rw_entry_iceberg_insert(transactional_splinterdb *txn_kvsb, rw_entry *entry)
{
   ...
   bool is_new_item = iceberg_insert_and_get_without_increasing_refcount(
      txn_kvsb->tscache,
      key_ht,
      (ValueType **)&entry->cache,
      platform_get_tid() - 1);
   platform_assert(entry->cache != &ts);
#else
   ...
   /* platform_assert(((uint64)entry->cache) % sizeof(txn_timestamp) == 0);
    */
...
{
   timestamp_set v1, v2;
   timestamp_set_load(entry->cache, &v1);
   if (v1.lock_bit) {
      return false;
   }
   v2          = v1;
   v2.lock_bit = 1;
   return timestamp_set_compare_and_swap(entry->cache, &v1, &v2);
}
...
rw_entry_unlock(rw_entry *entry)
{
   ...
   do {
      ...
      v2          = v1;
      v2.lock_bit = 0;
   } while (!timestamp_set_compare_and_swap(entry->cache, &v1, &v2));
}
...
RETRY_LOCK_WRITE_SET:
{
   ...
   for (uint64 i = 0; i < num_writes; ++i) {
      commit_ts = MAX(commit_ts, write_set[i]->cache->rts + 1);
   }
   ...
   for (uint64 i = 0; !is_abort && i < num_reads; ++i) {
      ...
      if (r->rts < commit_ts) {
         ...
         do {
            is_success = TRUE;
            timestamp_set_load(r->cache, &v1);
            bool is_wts_different = r->wts != v1.wts;
            // txn_timestamp rts                  = timestamp_set_get_rts(&v1);
            bool is_locked_by_another =
               v1.rts <= commit_ts && v1.lock_bit && !rw_entry_is_write(r);
            if ((is_abort = (is_wts_different || is_locked_by_another))) {
               break;
            }
            if (v1.rts < commit_ts) {
               ...
               v2         = v1;
               v2.rts     = commit_ts;
               is_success = timestamp_set_compare_and_swap(r->cache, &v1, &v2);
            }
         } while (!is_success);
      }
   }
   ...
   if (!is_abort) {
      ...
      for (uint64 i = 0; i < num_writes; ++i) {
         ...
         timestamp_set v1, v2;
         do {
            timestamp_set_load(w->cache, &v1);
            v2.lock_bit = 0;
            v2.rts      = commit_ts;
            v2.wts      = commit_ts;
         } while (!timestamp_set_compare_and_swap(w->cache, &v1, &v2));
      }
   } else {
      for (uint64 i = 0; i < num_writes; ++i) {
         rw_entry_unlock(write_set[i]);
...
#endif
   } while (!timestamp_set_compare_and_swap(entry->cache, &v1, &v1));
   // XXX: The code below is too slow. Why?
   // timestamp_set_load(entry->cache, &v2);
